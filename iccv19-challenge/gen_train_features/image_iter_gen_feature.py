...
class FaceImageIter(io.DataIter):
    ...
    def __init__(self, batch_size, data_shape,
        ...
        train_features = np.fromfile("train_features", dtype=np.float32)
        print("loading train_features_flip ...")
        train_features_flip = np.fromfile("train_features_flip", dtype=np.float32)
        self.train_features = train_features.reshape(5179510, 512)
        self.train_features_flip = train_features_flip.reshape(5179510, 512)

        # self.train_features = train_features.reshape(102, 512)
        # self.train_features_flip = train_features_flip.reshape(102, 512)

        #### np.loadtxt
        # self.train_features = np.loadtxt("train_features")
        # self.train_features = np.loadtxt("train_features_flip")
        # print()
    ...
    def next(self):
        ...
        try:
            while i < batch_size:
                ...
                features_data[i, :] = self.train_features[self.seq[self.cur-1]-1, :]
                ...
                if self.rand_mirror:
                  ...
                  if _rd==1:
                    ...
                    features_data[i, :] = self.train_features_flip[self.seq[self.cur - 1] - 1, :]
