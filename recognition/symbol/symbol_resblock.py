...
def BottleNeckV3(data, num_in, num_mid, num_out, name, first_block=False, stride=(1, 1), num_group=1, use_se=False, ratio=-1):

    sym = symbol_basic
    # print('sym = symbol_basic')
    if ratio >= 0:
        num_mid = (num_mid - int(ratio * num_mid), int(ratio * num_mid))
        num_out = (num_out - int(ratio * num_out), int(ratio * num_out))
        sym = symbol_octconv  # overwrite operators
        # print('sym = symbol_octconv')

    out = sym.BatchNorm(data=data, name=('%s_bn' % name))
    out = sym.Conv_BN_ACT(out, num_mid, (3, 3), pad=(1, 1), stride=(1,1), name=('%s_conv1' % name), act_type='relu')

    out = sym.Conv_BN(out, num_mid, (3, 3), pad=(1, 1), stride=stride, name=('%s_conv2' % name))

    # optional
    out = SE_Block(sym=sym, data=out, num_out=num_mid, name=('%s_se' % name)) if use_se else out

    if first_block:
        data = sym.Conv_BN(data, num_mid, (1, 1), pad=(0, 0), stride=stride, name=('%s_sc' % name))
