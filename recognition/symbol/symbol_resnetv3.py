...
from symbol_resblock import BottleNeckV3
...
def get_before_pool(depth, group=1, scaler=1., ratio=-1, use_fp16=False, use_se=False):
    ...
    conv1_x = Conv_BN_ACT(data=data, num_filter=num_in, kernel=(3, 3), pad=(1, 1), stride=(1, 1), name='conv1')
    conv1_x = mx.symbol.Pooling(data=conv1_x, pool_type="max", kernel=(3, 3), pad=(1, 1), stride=(2, 2), name='pool1')

    # stage 2
    for i in range(1, k_sec[2] + 1):
        conv2_x = BottleNeckV3(data=(conv1_x if i == 1 else conv2_x),
    ...
    for i in range(1, k_sec[3] + 1):
        conv3_x = BottleNeckV3(data=(conv2_x if i == 1 else conv3_x),
                               num_in=(num_in if i == 1 else num_out),
                               num_mid=num_mid,
                               num_out=num_out,
                               name="L3_B%02d" % i,
    ...
    for i in range(1, k_sec[4] + 1):
        conv4_x = BottleNeckV3(data=(conv3_x if i == 1 else conv4_x),
                               num_in=(num_in if i == 1 else num_out),
                               num_mid=num_mid,
                               num_out=num_out,
                               name="L4_B%02d" % i,
...
def get_linear(num_classes, depth, group=1, scaler=1., ratio=-1, dropout=0., use_fp16=False, use_se=False):
    ...
    # - - - - -
    pool5 = mx.symbol.Pooling(data=before_pool, pool_type="avg", kernel=(7, 7), stride=(1, 1), global_pool=True,
                              name="global-pool")
